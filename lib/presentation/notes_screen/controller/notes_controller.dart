...
import 'package:secondbrain_app/presentation/notes_screen/models/notes_model.dart';
...
import 'package:secondbrain_app/core/constants/NotesConstant.dart';
import 'package:secondbrain_app/data/models/note_query/post_query_resp.dart';
import 'package:secondbrain_app/data/models/note_query/post_query_req.dart';
...
class NotesController extends GetxController {
  ...
  onSelected(dynamic value) {
    ...
    notesModelObj.value.dropdownItemList.forEach((element) {
      element.isSelected = false;
      if (element.id == value.id) {
        element.isSelected = true;
      }
    });
    notesModelObj.value.dropdownItemList.refresh();
  }
  ...
  void _onCreateQuerySuccess() {
    Get.find<PrefUtils>().setNotesDBProperty(postQueryResp.results!);
    var result= Get.find<PrefUtils>().getNotesDBProperty();
    for(var index in result!){
      this.callFetchAooH(
      successCall: _onFetchAooHSuccess,
      errCall: _onFetchAooHError,
      pageId:index.id,
      noteId:index.properties!.notes!.id,
      date:index.properties!.date!.id,
      flag:result.length == index ?true : false
    );
    }
  ...
  void onFetchAooHSuccess(var response,var flag) {
   ...
   List<NotesItemModel> notePropertyList= [];
   for(var i in propertyArray){
     for(var index in i.results!){
      var noteProperty=NotesItemModel();
  ...
  notesModelObj.value.notesItemList.value= notePropertyList;  
